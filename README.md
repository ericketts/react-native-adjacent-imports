## react-native-adjacent-imports
The layout of this simple example project is as follows:
* `src/shared/` containing shared code
...
* `package.json`s must be added to each sub-module of the `src/` dir (or at least the ones you want to absolutely import in react-native). All you need within the `package.json` is `"name": "dir_name"`, where `dir_name` is the name of the module directory.
* Adding `rn-cli.config.js` file to the react-native module directory; this file tells metro what search paths to use. Check the comments within for more detail.

And that should do it!
As you will see, the `App.js` of both the `web/` and `rn/` project are absolutely importing dependencies from adjacent modules; and we don't have to duplicate dependencies needed by react-native if they're already in our root react project (such as how the react-native package.json doesn't specify react as a dependency, since it can use the root project's copy).

Now getting this all working with flow is a special hell of it's own :)
